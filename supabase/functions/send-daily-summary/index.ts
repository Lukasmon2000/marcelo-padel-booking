...
serve(async (req) => {
  ...
  try {
    ...
    const { data: bookings, error } = await supabase.rpc("get_pending_today_email_summaries");
    ...
    for (const booking of bookings || []) {
      try {
        ...
        await sendBrevoEmail({
          to: email,
          subject: "Hoy tienes clase de pádel",
          htmlContent: `
            <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111827;">
              <h2>🎾 Hoy tienes clase de pádel</h2>
              <p>Hola ${userName},</p>
              <p>Te recordamos tu clase de hoy.</p>
              ...
              <p>Nos vemos en Marcelo Pádel.</p>
            </div>
          `,
          textContent: `Hoy tienes clase de pádel\nHola ${userName}\nDía: ${dayName} ${date}\nHora: ${startTime}${endTime ? ` - ${endTime}` : ""}\nPista: ${courtName}\nMonitor: ${monitor}`,
        });

        await supabase
          .from("bookings")
          .update({ daily_summary_sent_at: new Date().toISOString(), daily_summary_email_error: null })
          .eq("id", booking.booking_id)
          .is("daily_summary_sent_at", null);
        ...
        await supabase
          .from("bookings")
          .update({ daily_summary_email_error: err instanceof Error ? err.message : "Error desconocido" })
          .eq("id", booking.booking_id);
      }
    }
