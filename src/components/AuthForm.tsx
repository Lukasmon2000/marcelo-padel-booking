...
function getAuthErrorMessage(error: unknown) {
  ...
  if (message.includes("email rate limit exceeded")) {
    return "No se puede crear la cuenta ahora porque Supabase ha bloqueado temporalmente los emails de confirmación. Espera unos minutos o desactiva la confirmación por email mientras pruebas.";
  }
...
export function AuthForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const { normalizedEmail, cleanPassword, cleanFullName, cleanPhone } = validated;
  ...
  <div className="flex items-start space-x-2">
    <Checkbox
      ...
      onCheckedChange={(checked) => setMarketingEmailsEnabled(checked === true)}
    />
    ...
    >
      Quiero recibir avisos y novedades sobre clases disponibles, horarios y actividades de la escuela.
    </Label>
  </div>
)}
